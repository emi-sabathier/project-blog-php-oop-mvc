...
<div>
    <form action="index.php?action=updatePost&postId=<?= $post['id'] ?>" method="post" class="form-group">
        <label for="title">Titre du post :</label><br />
        <input type="text" id="title" name="title" value="<?= $post['title'] ?>" class="form-control form-control-md"><br />
        <label for="content">Votre texte : </label><br />
        <textarea id="postArea" name="content" class="form-control form-control-md"><?= $post['content'] ?></textarea>
